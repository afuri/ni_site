...
Сделал выбор ширины изображения перед загрузкой (200–1600 и “Исходный
  ...
  - Клиентский ресайз (как сейчас): быстро и просто, снижает трафик/время
    загрузки и хранение.
  - Серверный ресайз (если хотите единый контроль и без доверия к клиенту):
    загружать оригинал, а сервер/хранилище генерирует нужные размеры (например,
    через worker + MinIO). Это дороже в реализации.

  Если хотите, могу добавить серверный вариант отдельным заданием.

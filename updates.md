...
Сделал нужную перестановку для мобильной версии на странице олимпиады.

  Изменения:

  - frontend/apps/main/src/pages/OlympiadPage.tsx
      - добавил мобильный блок навигации (две строки: «< >» и «Начало /
        Последняя»)
      - в десктопной навигации текст «Последнее» → «Последняя»
  - frontend/apps/main/src/styles/olympiad.css
      - для мобильных: заголовок в 3 строки (название / таймер+логин / ?
        +Завершить)
      - скрываю старые nav‑блоки слева/справа и показываю мобильный

  Что именно получилось на мобиле:

  1. Верхнее меню:
      - строка 1: название олимпиады (по центру)
      - строка 2: таймер + логин
      - строка 3: кнопки “?” и “Завершить”
  2. Нижняя навигация:
      - строка 1: “<” и “>”
      - строка 2: “Начало” и “Последняя”


Сделал. Теперь блок «Личные данные» скрыт под плашку‑кнопку, по клику
  раскрывается/скрывается.

  Что изменил:

  - frontend/apps/main/src/pages/CabinetPage.tsx
      - добавил isProfileOpen и обернул форму в условный рендер
      - заголовок заменён на кнопку‑плашку
  - frontend/apps/main/src/styles/cabinet.css
      - стили для .cabinet-toggle, hover/focus, размер на мобильных


- обернул каждый блок кабинета в отдельный контейнер с горизонтальным
    скроллом;
  - на мобиле сделал блоки уже (не во всю ширину);
  - верхнее меню на мобиле привёл к виду главной страницы: скрывается текст в
    логотипе и навигация, высота шапки уменьшается.
  ...
  Что в итоге на мобиле:

  - каждый блок «Личный кабинет / Результаты / Профиль / Сопровождение» в своём
    scroll‑контейнере;
  - блоки стали компактнее (ширина min(520px, 88vw)), центрируются;
  - верхняя шапка как на главной: только логотип и кнопка пользователя, без
    текста и навигации.



Сделал — теперь окна создания задания и создания олимпиады не закрываются по
  клику за пределами.
...
Сделал оба правки.
  ...
  - Убрал “липкость” верхнего меню на мобильном экране страницы олимпиады.
    Теперь оно уходит при скролле.
  - В просмотре попытки в личном кабинете отображается только введённый ответ
    (без JSON и без “верно/неверно”).

  Изменённые файлы

  - frontend/apps/main/src/styles/olympiad.css
      - Мобильное правило: .olympiad-page .layout-header { position: static; }
  - frontend/apps/main/src/pages/CabinetPage.tsx
...
1.3.13 instruction message

Изменил инструкцию перед олимпиадой

1.4.0

Добавил admin‑виджет “Статистика” с числами активных попыток и
  уникальных пользователей, обновляется по кнопке.

  Что сделано:

  - Новый backend endpoint GET /api/v1/admin/stats/attempts (только для admin)
      - active_attempts — все активные попытки
      - active_attempts_open — активные и не истекшие (deadline > now)
      - active_users_open — уникальные пользователи в активных попытках
  - В админке (страница “Отчеты”) добавлен блок “Статистика” сверху + кнопка
    “Обновить”.
  - Добавлены CSS‑стили карточек.

  Файлы:

  - backend/app/api/v1/admin_stats.py (новый)
  - backend/app/schemas/admin_stats.py (новый)
  - backend/app/api/v1/router.py (подключён router)
  - frontend/apps/admin/src/pages/ReportsPage.tsx
  - frontend/apps/admin/src/styles/admin.css

Сделал выбор ширины изображения перед загрузкой (200–1600 и “Исходный
  размер”). Масштабирование по ширине, пропорции сохраняются. Если выбран
  “Исходный размер” или картинка меньше выбранной ширины — загрузится оригинал
  без ресайза.

  Изменения:

  - frontend/apps/admin/src/pages/TasksPage.tsx
      - добавлено состояние imageResizeWidth
      - выбор ширины в форме
      - логика ресайза использует выбранную ширину

1.4.2

сейчас токены/пользователь хранятся в localStorage, он переживает
  закрытие браузера. Сервер может инвалидировать токен, а клиент всё ещё
  “думает”, что логин есть → при защищённом действии 401 и нужен ручной вход.

  Я сделал так, чтобы в main‑приложении токены и user хранились в
  sessionStorage. Он очищается при закрытии вкладки/браузера → сессия на
  клиенте сбрасывается автоматически.

  Изменения:

  - frontend/apps/main/src/utils/authStorage.ts — новый хелпер, использует
    sessionStorage.
  - frontend/apps/main/src/App.tsx
  - frontend/apps/main/src/pages/HomePage.tsx
  - frontend/apps/main/src/pages/OlympiadPage.tsx
...
Сделал: теперь на 401 в личном кабинете вызывается signOut() сразу, как и на
  главной и на странице олимпиады.

  Изменено:

  - frontend/apps/main/src/pages/CabinetPage.tsx — добавил onAuthError: signOut
    в createApiClient.

1.4.3 fix task picture resize function

Сделал рабочий вариант: теперь после выбора ширины картинка перезагружается и
  пересохраняется с новым размером (даже если ширину выбрали уже после загрузки
  файла).

  Что изменил в frontend/apps/admin/src/pages/TasksPage.tsx:

  - сохраняем оригинальный dataUrl/тип/имя;
  - при смене ширины выполняется повторный ресайз + аплоад;
  - очистка сбрасывает исходник.

  Теперь порядок такой:

  1. загрузили файл;
  2. если поменяли ширину — происходит повторная обработка и загрузка;
...
График активных попыток

Сделал: если попытка уже завершена (status != active), пользователь сразу
  видит окно завершения и не имеет доступа к заданиям/ответам. Окно ведёт в
  личный кабинет.
  ...
  - При открытии завершённой олимпиады автоматически запрашивается результат
    (если доступен) и показывается модалка.
  - Просмотр заданий/ответов полностью скрыт.

1.5.4

Таблица  вместо графика

 Готово. Добавил предпросмотр олимпиад в админке без старта попытки и без
  сохранения ответов.

  Что сделано

  - В списке олимпиад появилась кнопка «Предпросмотр».
  - Открывается модалка с заданиями как у ученика:
      - варианты ответов показаны (radio/checkbox) и отключены
      - короткий ответ — отключённое поле
      - изображения отображаются с учётом image_position
  - Никаких попыток/ответов не создаётся.
...
1.6.4 lookup в личном кабинете не запускался при открытии

Заблокировано изменение класса
Скрыл пробную олимпиаду
Отображение кооличества завершенных олимпиад
Сделал так, чтобы lookup в личном кабинете не запускался при открытии
  страницы, а только после ввода в поля «Город»/«Школа».
  ...
  - Добавлены флаги cityLookupEnabled и schoolLookupEnabled.
  - Lookup запускается только после первого onChange соответствующего поля.
  - После загрузки профиля, сохранения или отмены — флаги сбрасываются, чтобы
    не дергать lookup.

1.6.5 disabled 401 autosave

на 401 теперь сразу signOut() и автосейв останавливается.
  ...
  - Добавлен флаг isAuthInvalid.
  - onAuthError теперь ставит этот флаг и делает signOut().
  - saveAnswer() и submitAttempt() сразу выходят, если isAuthInvalid — больше
    не бомбят БД.
  - Авто‑submit при remainingSeconds === 0 тоже отключается при isAuthInvalid.

Что изменится для пользователя:

  - Если сессия истекла или токен сломан, его сразу разлогинит, и
    автосохранение ответов остановится.
  - Ответы, которые успели уйти до 401, сохранятся.
  - Ответы после 401 не будут сохранены (это уже и раньше не сохранялось, но
    теперь не будет бесконечных попыток).

  Риск:

  - Пользователь может потерять последние введённые ответы после истечения
    сессии.
    Но это уже фактически происходило (401), просто теперь это не спамит БД.

  Плюс:

  - Нет лавины 401 → меньше нагрузки, меньше очередей, стабильнее сервис.



1.6.6

Сделано. Теперь в админке список заданий для олимпиады грузится полностью, и
  поиск работает по всему пуллу.

  Что поменял:

  - frontend/apps/admin/src/pages/OlympiadsPage.tsx
  - loadTaskCatalog() теперь:
      - берёт total через /admin/tasks/count
...
1.6.8
 ...
 Добавил валидацию для short_text с subtype = "int": при вводе
  нецелого числа показывается предупреждение, а сохранение/уход с задания/
  сабмит пропускают такой ответ.

  Изменения:

  - frontend/apps/main/src/pages/OlympiadPage.tsx — расширил payload с subtype,
    добавил shortTextErrors, валидацию ^-?\\d+$, показ ошибки под инпутом и
    блок сохранения/сабмита при неверном формате.

  Если хочешь, могу добавить более мягкое поведение (например, не блокировать
  сохранение, а только предупреждать).

  предупреждение остается, но сохранение/переход/submit не блокируются.

  Изменено: frontend/apps/main/src/pages/OlympiadPage.tsx — убраны ранние
  return при ошибке валидации, оставлено только отображение предупреждения.

1.6.8.1

Теперь строгая валидация:

  - Для int: нельзя сохранить ответ и нельзя перейти к следующему заданию, пока
    не введено корректное целое число. Сообщение:
    «В ответ можно указать только целое число.»
  - Для float: нельзя сохранить и нельзя перейти дальше при неверном формате.
    Сообщение:
  ...
  - сохранение (Сохранить ответ) блокируется;
  - переход на другое задание блокируется при ошибке;
  - подсказка выводится под инпутом.

1.6.8.2

Сделал. Теперь в поле ответа placeholder зависит от subtype:
...
1.7.0 auto-estimation expired attempts

Готово: добавил оценивание для expired‑попыток в teacher/admin просмотре и
  автопересчёт при выдаче результатов в кабинете. Теперь при просмотре попытки
  администратором/учителем автоматически пересчитываются баллы и формируются
  attempt_task_grades, а в личном кабинете expired‑попытки с пустой оценкой
  приводятся в порядок при выдаче списка.

  Изменения:

  - backend/app/services/teacher.py — при открытии попытки учителем/админом,
    если попытка просрочена или expired без оценок, выполняется пересчёт,
    записываются оценки и graded_at.
  - backend/app/services/attempts.py — get_attempt_view теперь пересчитывает и
    для expired без оценок; list_results автопересчитывает такие попытки перед
    выдачей.

Убрал зависимость таймера от времени


1.8.0 Fix сброс авторизации при олимпиаде

Сделал правки, чтобы максимально предотвратить «слёт» авторизации во время
  олимпиады:

  - Увеличил дефолт TTL access‑token до 60 минут в backend/app/core/config.py
    (на случай, если .env не подхватится).
  - Добавил автorefresh в олимпиаде: сразу при входе и затем каждые 45 минут,
    только в активной попытке, без принудительного logout при временной ошибке.
    Также refresh пропускается, если вкладка скрыта, чтобы не устроить гонку
    токенов в фоне.
    Файл: frontend/apps/main/src/pages/OlympiadPage.tsx



1.8.1 Confirm window lock

Сделал блокировку подтверждающего окна «Завершить» на время отправки или 3
  секунды — закрытие по крестику/фону и кнопка «Вернуться» не работают, пока
  блок активен.

  Что изменено

  - frontend/apps/main/src/pages/OlympiadPage.tsx
      - добавлены isFinishLocked и таймер блокировки на 3 секунды
      - handleFinishConfirm() ставит блокировку, запускает таймер, вызывает
        submitAttempt, снимает блокировку при завершении
      - у модалки отключено закрытие по фону на время блокировки
        (closeOnBackdrop)
      - onClose игнорируется, пока блокировка активна
      - кнопки «Завершить» и «Вернуться» отключаются при блокировке

  Результат

  - после нажатия «Завершить» окно нельзя закрыть и нельзя нажимать другие
    кнопки до:
...
Сделал более надёжный фикс: убрал зависимость handleAuthError от attemptView
  (через useRef), чтобы TDZ больше не мог возникать даже при перестановке кода.
  Нужно обновить и пересобрать фронт.



1.8.6 regrade 2 class
...
1.8.6.6 invalid olymp start solving
...
1.8.6.8 invalid olymp start solving

Внёс правки во фронтенд, чтобы убрать массовый мгновенный submit.
...
Добавлен безопасный парсер серверных дат с принудительным UTC при отсутствии таймзоны: parseServerDateToMs (OlympiadPage.tsx (line 76)).
Таймер теперь считает время через этот парсер и не запускает автозавершение на некорректном/пустом времени (OlympiadPage.tsx (line 255)).
Авто-submit теперь выполняется только если попытка active и таймер ранее был положительным (защита от мгновенного submit на первом тике):
OlympiadPage.tsx (line 409).
submitAttempt дополнительно защищён: не отправляет submit если попытка уже не active:
OlympiadPage.tsx (line 571).

Принял замечания и исправил их в OlympiadPage.tsx.

Что изменено:

Исправлен парсинг +00 из Postgres
OlympiadPage.tsx (line 76)
Теперь корректно нормализуются +00 и +0000 в +00 (line 0), NaN больше не возникает из-за +00Z.

Убран жёсткий форс на UTC для “наивных” дат
OlympiadPage.tsx (line 95)
Если таймзона не указана, сначала используется локальная семантика (Date.parse как есть), и только если парсер не принимает строку — fallback к UTC.

Добавлен безопасный fallback для remainingSeconds=0 на первом тике
OlympiadPage.tsx (line 437)
Если таймер сразу пришёл в 0 и до этого не было положительного значения, авто-submit не срабатывает мгновенно: ставится отложенная попытка на 5 секунд (с отменой при изменении состояния). Это убирает мгновенное закрытие и не оставляет попытку “висеть” навсегда.


1.8.7 Тестирование по коду

Изменения:

Добавил новый раздел «Тестирование по коду» сразу после блока «Организаторы и партнеры» в
HomePage.tsx.

В разделе добавил:

поле ввода с плейсхолдером код тестирования
кнопку Начать
вывод информации об олимпиаде под полем (если код валиден и олимпиада доступна сейчас):
название
класс (age_group)
дата проведения
длительность
Реализовал логику кода:

код = 1000000 + id_олимпиады
из кода вычисляется id
проверка делается через /olympiads?limit=500&offset=0
если олимпиада найдена, но недоступна по времени — информация не показывается, выводится ошибка
если доступна — показывается карточка с данными
Кнопка Начать теперь запускает попытку напрямую по найденной олимпиаде:

POST /attempts/start с olympiad_id
при успехе переход на /olympiad?attemptId=...
сохранены проверки: авторизация, роль student, подтвержденный email
Добавил стили для нового блока в
home.css
(включая адаптив под мобильные).
...
1.9.0 extra level
спрятал через hidden раздел Олимпиады




1.9.1 user id in result table

Добавил отображение ID пользователя в таблице результатов попыток в админке.
...
Добавлена колонка ID пользователя в таблицу попыток.
Добавлено значение item.user_id в строки таблицы.
...
1.13.6 UI result page fix

Исправил поведение кнопок, теперь работает как нужно.

Что поменял в ResultsArchivePage.tsx:

Убрал открытие окна через window.open("", ...) (из-за этого и была блокировка popup).
Добавил предварительную проверку существования PDF при загрузке страницы:
сначала HEAD,
если 405 — fallback GET с Range: bytes=0-0.
Если файл найден:
рендерится обычная ссылка <a target="_blank" rel="noreferrer"> — как на главной для Положение/Регламент.
Если файла нет:
рендерится кнопка, по клику открывается модалка:
Информация будет размещена позже.
Итог:

Блокировка всплывающих окон исчезает.
Модалка для отсутствующих файлов показывается.
Существующие PDF открываются обычным способом в новой вкладке.

1.13.7  Ui fix result page

Изменил поведение на странице результатов:

Если PDF-файл отсутствует, кнопка не отображается (hidden логически).
Модальное окно полностью удалено.
Если файл есть, остается обычная ссылка и открытие в новой вкладке (как на главной).
Файл:
...
1.16.0 Announcement to users

```Давай я детально распишу, что требуется. Дай только рекомендации. Необходимо чтобы в личном кабинете некоторых участников появилось одно или два объявления. Содержание объявлений и время размещения можно настраивать из админ панели. Все ученики будут разделены на 22 группы. 21 группа, у которых будут объявления и остальные, у которых не будет  объявления. 
Имеется следующая необходимость: Надо разделить всех победителей по площадкам для приглашения на участие в очном этапе по географическому признаку. Очных этапа будет два: по математике и по инфоорматике. Соответственно, некоторые будут приглашены только на матеамтику на свою площадку, некоторые только на информатику. А у некотоорых будут два приглашения. Список групп (user_id, group_number) для математики в файле csv, список групп для информатики (user_id, group_number) в другом файле csv. 
Для каждой группы свою объявление с информацией по очной площадке. Но есть и общая часть в объявлении. 
Пользователям, user_id которых нет в таблице групп, можно вывести одно общее объявление, а можно не выводить.
Порекомендуй решение```

Рекомендую сделать это как “кампанийную систему объявлений” с таргетингом по предмету и группе, без хардкода в `users`.

**1. Модель данных**
- `announcement_campaigns`
  - `id`, `code` (например `final_2026`)
  - `title_default`
  - `common_text` (общая часть)
  - `is_active`
  - `starts_at`, `ends_at`
- `announcement_groups`
  - `id`
  ...
  - `group_number` (1..21)
  - `group_title`
  - `group_text` (переменная часть: площадка, адрес, время)
  - `starts_at`, `ends_at` (можно переопределять на группу)
  - `is_active`
  - `UNIQUE(campaign_id, subject, group_number)`
- `user_group_assignments`
  - `campaign_id`
  - `user_id`
  - `subject` (`math` | `cs`)
  - `group_number`
  - `source_file`, `assigned_at`
  - `UNIQUE(campaign_id, user_id, subject)`  
  Это позволит одному пользователю иметь 0/1/2 объявления (math/cs).
- `campaign_fallback`
  - `campaign_id`
  - `enabled` (показывать ли “общее” тем, кто не попал в группы)
  - `title`, `text`

**2. Логика показа (backend)**
- Эндпоинт: `GET /cabinet/announcements/me`
- Для текущего пользователя:
  - взять назначения из `user_group_assignments` по активной кампании
  - присоединить тексты из `announcement_groups`
  - собрать итоговые карточки: `common_text + group_text`
- Если назначений нет:
  - если `campaign_fallback.enabled=true` → вернуть 1 общее объявление
  - иначе вернуть пусто
- Ограничить максимум 2 карточки (math + cs) по вашей бизнес-логике.

**3. Импорт из 2 CSV**
- Отдельные импорт-процедуры:
  - `import_math_groups.csv`
  - `import_cs_groups.csv`
- Формат: `user_id,group_number`
- Валидировать:
  - `user_id` существует и роль `student`
  - `group_number` в диапазоне 1..21
- Импорт через upsert, чтобы обновления были идемпотентны.
- Хранить результат импорта (кол-во строк, ошибки) и лог.

**4. Админ-панель**
- Экран “Кампания объявлений”:
  - включить/выключить кампанию
  - общий текст (`common_text`)
  - fallback объявление (вкл/выкл + текст)
- Экран “Группы”:
  - 21 группа для `math`, 21 для `cs`
  - редактирование `group_text`, дат, активности
- Экран “Импорт назначений”:
  - загрузка 2 CSV отдельно
  - превью и валидация до применения
  - отчёт по ошибкам

**5. Почему это оптимально**
- Не трогаем `users`, схема расширяемая и сезонная.
- Легко настраивать время публикации “раньше/позже”.
- Поддерживает 0/1/2 объявления на пользователя.
- Прозрачный аудит и повторные импорты без дубликатов.

**6. Практические нюансы**
- Добавить флаг версии кампании (`code`), чтобы не смешивать годы.
- Кэшировать ответ `announcements/me` на короткое время (например 30–60 сек).
- В UI раздел “Объявления” показывать только при непустом массиве ответа.


Backend-часть реализована.

Что добавил:

Новые сущности объявлений
announcement.py
announcement_campaigns
announcement_group_messages
announcement_assignments
announcement_campaign_fallbacks
Alembic migration
4f9c2a7b1d3e_add_announcement_campaigns.py
Репозиторий и бизнес-логика
announcements.py
выдача объявлений пользователю (0/1/2; math/cs; fallback если назначений нет)
CRUD-операции кампаний/групп/fallback
импорт назначений из CSV с валидацией и статистикой
Схемы API
announcements.py
API для админки
admin_announcements.py
GET /api/v1/admin/announcements/campaigns
POST /api/v1/admin/announcements/campaigns
PATCH /api/v1/admin/announcements/campaigns/{campaign_id}
GET /api/v1/admin/announcements/campaigns/{campaign_id}/groups
PUT /api/v1/admin/announcements/campaigns/{campaign_id}/groups
GET /api/v1/admin/announcements/campaigns/{campaign_id}/fallback
PUT /api/v1/admin/announcements/campaigns/{campaign_id}/fallback
POST /api/v1/admin/announcements/campaigns/{campaign_id}/import/{subject} (CSV upload, subject=math|cs)
API для кабинета пользователя
users.py
GET /api/v1/users/me/announcements
для student возвращает объявления, для других ролей []
Подключение роутера + коды ошибок + тестовый bootstrap импорт модели
router.py
error_codes.py
env.py
conftest.py
...
py_compile на изменённых backend-файлах проходит.
Полноценные runtime-тесты локально не запускал (в окружении нет установленных backend-зависимостей).
Команды применения на сервере:
...
1.7.0 Announcement mainpage 

Изменения:

Перенес блок «Тестирование по коду» после «Об олимпиаде».
Файл: HomePage.tsx

Сделал FAQ свернутым под плашкой-кнопкой (без + и без стрелки на самой плашке).
Файл: HomePage.tsx
Файл: home.css

Добавил новый блок перед «Об олимпиаде»:

слева красивый выделенный блок с контентом из announcement.md,
справа встроенный виджет из clock_vidget.md,
без заголовка секции.
Файл: HomePage.tsx
Файл: home.css
...
1.9.7.0 Examples_students_works
...
2.1.3 school_database_fix

- Остальные изменения сохранены:
  - обязательные поля и placeholders формы;
  - инструкция и прокрутка формы;
  - редактирование страны и города администратором;
  - создание города только после одобрения с защитой от дублей;
  - возможность пользователю менять выбранные регион и школу;
  - backend-валидация новых обязательных полей.

  Готово. Теперь при нажатии «Открыть» заявка школы отображается в отдельном модальном окне.
  На странице «Школы»:
- Заголовки обеих таблиц закреплены и остаются видимыми при вертикальной прокрутке.
- Для справочника школ и списка заявок добавлены кнопки:
  - «В начало»;
  - «Назад»;
  - «Вперёд»;
  - «В конец».
- Для заявок добавлен подсчёт общего количества через отдельный API endpoint.
...
2.1.6 new_school_database_fix

Реализована кнопка «Показать еще» в выборе школы при регистрации и в личном кабинете.

Что изменено:

- первые 20 школ отображаются как раньше;
- если есть дополнительные подходящие активные школы, появляется кнопка «Показать еще»;
- каждое нажатие подгружает следующие 20 записей;
- при изменении региона или поискового запроса пагинация сбрасывается;


2.1.7 fix_admin_panel_school_aprove

Реализовано именно указанное поведение:

- в заявке показывается регион, первоначально выбранный пользователем;
- администратор может выбрать правильный регион;
- после смены региона текущая выбранная школа и результаты поиска очищаются;
- новый поиск выполняется только среди школ исправленного региона;
- при одобрении пользователю записываются выбранная школа и её регион.

...
Реши проблему — в backend‑логике. Сейчас при истечении дедлайна попытка помечается
  ...
  - graded_at остаётся NULL ⇒ в кабинете дата пустая.
  - score_total/score_max остаются 0 ⇒ итог 0%.

  Это видно в backend/app/services/attempts.py:

  - get_attempt_view() авто‑expire при now > deadline_at;
...
Применить на backend

Да, для полного устранения нужен бэкенд-фикс.

Почему:

Сейчас POST /attempts/{id}/submit доверяет клиенту и принимает сабмит сразу после старта.
attempts.py (line 463)
В submit() нет проверки “минимального времени с started_at” и нет требования наличия ответов.
attempts.py (line 533)
Значит любой баг фронта/скрипт/повторный запрос снова даст мгновенное закрытие, даже если фронт исправлен.
Минимум, что стоит добавить на бэке:

Guard в submit: если status=active, now-started_at < N (например 5 сек) и answers_cnt=0 → 409 (attempt_submit_too_early).
Rate limit для /attempts/{id}/submit (по аналогии с answers).
attempts.py
(Опционально) Разрешать ранний пустой сабмит только с явным флагом подтверждения (force_empty_submit=true) из модалки “Завершить”.
Если не менять бэк, проблема может повториться.
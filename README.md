...
- API‑конвенции: `API_CONVENTIONS.md`
- Роли и права: `ROLES.md`
- Удаление олимпиад админом: см. `Service_commands.md`
...
- Может менять разрешённые данные ученика и класс. Если у ученика уже выбран канонический регион и школа (`school_status=selected`), учитель не может менять эту географию; это может сделать только администратор
...
- region_id - обязательный выбор региона из справочника
- school_id - выбор школы из справочника выбранного региона; город определяется школой автоматически
- school_not_found - признак отсутствия школы в справочнике; после регистрации пользователь подаёт отдельную заявку на школу
...
- region_id - обязательный выбор региона из справочника
- school_id - выбор школы из справочника выбранного региона; город определяется школой автоматически
- school_not_found - признак отсутствия школы в справочнике; для дошкольника (`grade=0`) школа не требуется
...
Каноническая география пользователя хранится через `region_id`, `school_id` и
`school_status`. Строки `country`, `city`, `school` временно сохранены как legacy
для проверки миграции и не используются как источник истины. Пользователь со
статусом `selected` не может изменить регион или школу сам; администратор может.
Поле `coins` хранит неотрицательный баланс внутренней валюты.

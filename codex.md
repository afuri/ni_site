...
## 0) Обязательная Git-политика версии 2.0

- Разрешённая рабочая ветка проекта: только `codex/v2-development`.
- Перед любыми изменениями агент обязан проверить текущую ветку командой `git branch --show-current`.
- Если активна другая ветка, агент должен остановиться и сообщить пользователю. Нельзя автоматически переключать ветки или переносить незакоммиченные изменения.
- Запрещено автоматически выполнять `git pull`, `git fetch`, `git push` и любые эквивалентные команды синхронизации с удалённым репозиторием.
- Запрещено автоматически создавать, обновлять или объединять GitHub Pull Request.
- Запрещено автоматически выполнять merge, rebase или cherry-pick в `main`, переключаться на `main`, коммитить в `main` или отправлять изменения в `main`.
- Любой pull/fetch, push, создание Pull Request или операция интеграции допускается только после отдельного явного указания пользователя в текущей задаче с обозначением точной ветки и операции.
- Отсутствие запрета в задаче не считается разрешением. Общая просьба «завершить работу» также не разрешает сетевые Git-операции.
...
- пользователь может менять собственные регион и школу, включая статус `selected`;
- teacher не может менять подтверждённую географию ученика из своей группы, admin может;
